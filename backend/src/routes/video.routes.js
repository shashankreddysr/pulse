...
// Use a writable folder on Render
  ...
  async (req, res, next) => {
    try {
      ...
      const videoDoc = await Video.create({
        ...
        filePath: req.file.filename,     // stored filename
        size: req.file.size,
      ...
      processVideo(videoDoc).catch(() => {});

      return res.status(201).json({ message: "Video uploaded", video: videoDoc });
    } catch (err) {
      next(err);
    }
  }
...
router.get("/stream/:id", auth, async (req, res, next) => {
  try {
    ...
    if (video.tenantId !== req.user.tenantId) return res.status(403).json({ message: "Forbidden" });

    const videoPath = path.join(uploadDir, video.filePath);
    if (!fs.existsSync(videoPath)) return res.status(404).json({ message: "File missing on server" });
